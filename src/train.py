...
from model import SignGRUClassifier
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, model_save_path, model_info_path, label2idx, idx2label):
    """
    Trains a model with given dataloaders.
    ...
    Args:
        ...
        model_info_path (str): Path to save model architecture info.
        label2idx (dict): Label to index mapping.
        idx2label (dict): Index to label mapping.

    Returns:
        tuple: (list of training losses, list of validation losses, training accuracies, validation accuracies)
    """
    best_val_acc = 0.0
